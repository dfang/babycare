class ProcessDoctorLicenceBackImageJob
  queue_as :urgent

  def perform(doctor)
    # 因为wx.chooseImage, wx.uploadImage 会把图片上传到微信的服务器，所以这里要从微信服务器下载了，然后存到自己的服务器
    url = "http://file.api.weixin.qq.com/cgi-bin/media/get?access_token=#{WxApp::WxCommon.get_access_token}&media_id=#{doctor.license_back_media_id}"
