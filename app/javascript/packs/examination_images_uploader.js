    ...
    $(document).on("click", ".picker", e => {
      ...
      const preview = ($files, localId) => {
        if (util.isAndroid()) {
          ...
          wx
            .getLocalImgDataAsync({ localId: localId })
            .then(res => {
      ...
      const appendToPlaceholder = img => {
        ...
        // $files.append($li);
        $files
          .parents(".weui-uploader")
          .find(".weui-uploader__files")
          .append($li);
        $li.append($gallery);
      ...
      const syncUpload = (localIds, serverIds) => {
        if (localIds.length > 0) {
          ...
          wx
            .uploadImageAsync({ localId: localId, isShowProgressTips: 1 })
            .then(res => {
          ...
          for (let i = 0, len = serverIds.length; i < len; i++) {
            ...
            let $input = $('<input type="hidden">')
              .addClass("media-id")
              .attr("name", mediaId_name)
              .attr("value", mediaId_value);
